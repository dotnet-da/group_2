...
namespace backend
{
    public class Login
    {
        ...
        public async Task<string> GetPassword(string username)
        {
            ...
            cmd.CommandText = @"SELECT ac_password  FROM accounts WHERE ac_username = @ac_username";
        ...
        {
            using var cmd = Db.Connection.CreateCommand();
            cmd.CommandText = @"SELECT ac_type  FROM accounts WHERE ac_username = @ac_username";
            cmd.Parameters.Add(new NpgsqlParameter
            {
                ParameterName = "@ac_username",
                DbType = DbType.String,
