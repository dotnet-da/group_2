...
namespace backend
{
    public class Ingredient
    {
        ...
        public async Task<Ingredient> FindOneAsync(int id_ingredient)
        {
            ...
            cmd.CommandText = @"SELECT * FROM zutaten WHERE zu_id = @id_ingredient";
        ...
        {
            using var cmd = Db.Connection.CreateCommand();
            cmd.CommandText = @"SELECT * FROM zutaten;";
        ...
        {
            using var cmd = Db.Connection.CreateCommand();
            cmd.CommandText = @"INSERT INTO zutaten (zu_name, zu_amount) VALUES (@zu_name, @zu_amount);";
            BindParams(cmd);
        ...
        {
            using var cmd = Db.Connection.CreateCommand();
            cmd.CommandText = @"UPDATE zutaten SET zu_name = @zu_name, zu_amount = @zu_amount WHERE zu_id = @zu_id;";
            BindParams(cmd);
            BindId(cmd);
        ...
        {
            using var cmd = Db.Connection.CreateCommand();
            cmd.CommandText = @"DELETE FROM zutaten WHERE zu_id = @zu_id;";
            BindId(cmd);
