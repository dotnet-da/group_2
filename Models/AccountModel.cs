...
namespace backend
{
    public class Account
    {
        ...
        public async Task<Account> FindOneAsync(int id_user)
        {
            ...
            cmd.CommandText = @"SELECT * FROM accounts WHERE ac_id= @ac_id";
        ...
        {
            using var cmd = Db.Connection.CreateCommand();
            cmd.CommandText = @"INSERT INTO accounts (ac_username, ac_password, ac_type) VALUES (@ac_username, @ac_password, @ac_type);";
            BindParams(cmd);
        ...
        {
            using var cmd = Db.Connection.CreateCommand();
            cmd.CommandText = @"UPDATE accounts SET ac_username = @ac_username, ac_password = @ac_password, ac_type = @ac_type WHERE ac_id = @ac_id;";
            BindParams(cmd);
            BindId(cmd);
        ...
        {
            using var cmd = Db.Connection.CreateCommand();
            cmd.CommandText = @"DELETE FROM accounts WHERE ac_id = @ac_id;";
            BindId(cmd);
